...
import {
  ...
  Heart, Menu, X, ChevronRight, Bell, UserCog,
} from 'lucide-react';
...
export const ROLE_COLORS: Record<string, string> = {
  ...
  verifier:   'bg-teal-500/20 text-teal-300',
  attendant:  'bg-slate-500/20 text-slate-300',
};

// Pages each built-in role can access (fallback when no custom permissions)
const ROLE_PAGES: Record<string, AdminPage[]> = {
  master:     ['dashboard','admins','users','caregivers','requests','financial','identity','logs','support','settings'],
  ...
  supervisor: ['dashboard','users','caregivers','requests','financial','identity','support','logs'],
  verifier:   ['dashboard','identity','logs'],
  attendant:  ['dashboard','support','users','logs'],
};

const PERM_TO_PAGES: Record<string, AdminPage[]> = {
  view_users:          ['users'],
  ban_users:           ['users'],
  approve_caregivers:  ['caregivers'],
  approve_documents:   ['identity'],
  view_requests:       ['requests'],
  view_financial:      ['financial'],
  approve_withdrawals: ['financial'],
...
function getAllowedPages(admin: AdminUser): AdminPage[] {
  if (admin.role === 'master') return ROLE_PAGES['master'];
  if (admin.permissions && admin.permissions.length > 0) {
    const pages = new Set<AdminPage>(['dashboard']);
    admin.permissions.forEach(p => (PERM_TO_PAGES[p] || []).forEach(pg => pages.add(pg)));
    return Array.from(pages);
  }
  return ROLE_PAGES[admin.role] || ['dashboard'];
}
...
export default function AdminLayout({ admin, page, onNavigate, onLogout, children }: Props) {
  ...
  const roleColor = ROLE_COLORS[admin.role] || ROLE_COLORS['support'];
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="px-3 py-4 border-t border-slate-800">
        <div className="flex items-center gap-3 px-3 py-2.5 rounded-xl bg-slate-800/50">
          ...
          <button onClick={onLogout} title="Sair" className="p-1.5 text-slate-500 hover:text-red-400 transition-colors flex-shrink-0">
            <LogOut size={15} />
          </button>
        </div>
      </div>
    ...
    <div className="min-h-screen bg-slate-950 flex">
      ...
      {sidebarOpen && (
        <div className="lg:hidden fixed inset-0 z-40 flex">
          ...
          <aside className="relative w-64 bg-slate-900 border-r border-slate-800 flex flex-col h-full z-50">
            <button onClick={() => setSidebarOpen(false)} className="absolute top-4 right-4 p-1.5 text-slate-400 hover:text-white">
              <X size={18} />
            </button>
            <SidebarContent />
          </aside>
      ...
      <div className="flex-1 lg:ml-60 flex flex-col min-h-screen">
        <header className="sticky top-0 z-10 bg-slate-900/95 backdrop-blur border-b border-slate-800 px-4 lg:px-6 py-3.5 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <button onClick={() => setSidebarOpen(true)} className="lg:hidden p-2 text-slate-400 hover:text-white transition-colors">
              <Menu size={20} />
            </button>
            <div>
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="hidden sm:flex items-center gap-2 pl-3 border-l border-slate-700">
              ...
              <div>
                ...
                <p className={`text-[10px] ${(roleColor.split(' ')[1]) || 'text-slate-400'}`}>{roleLabel}</p>
              </div>
            </div>
          </div>
        </header>
